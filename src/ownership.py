...
def assign_ownership(
    ...
    assignment_log_path = config.output_dir / "program_memory" / "owner_assignments.csv"
    ...
    for plan_dir in plans_dir.iterdir():
        ...
        if task_id in tasks_df["task_id"].values:
            tasks_df["owner"] = tasks_df["owner"].astype(object)
            tasks_df["notes"] = tasks_df["notes"].astype(object)
            tasks_df.loc[tasks_df["task_id"] == task_id, "owner"] = owner
            tasks_df.loc[tasks_df["task_id"] == task_id, "owner_role"] = role
            if notes:
    ...
    existing = safe_read_csv(assignment_log_path)
    combined = pd.concat([existing, new_record], ignore_index=True) if not existing.empty else new_record
    combined.to_csv(assignment_log_path, index=False)
    
    return assignment_log_path
